...
class BusinessService: BusinessServiceProtocol {
    ...
    func getDetailBusiness(_ idBusiness: Int, completion: @escaping BusinessHandler) {
        ...
        guard let url = URL(string: Constants.API.baseURL + Constants.API.apiVersion + Constants.API.language + Constants.API.project + "/business/\(idBusiness)?mode=dashboard") else {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                ...
                let businesses = try decoder.decode(BusinessDetailResponse.self, from: data)
                print("SUCCESS businesses detail: \(businesses)")
